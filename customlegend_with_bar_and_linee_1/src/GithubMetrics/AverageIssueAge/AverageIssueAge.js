...
class AverageIssueAge extends Component {
  constructor() {
    super();
    this.state = {
  ...
  callApiTriggerFunction = (params) => {
    ...
    this.responseDataProcess('TOTAL_ISSUES', params);
  };
  ...
  responseDataProcess = (url, params) => {
    ...
    const responseProcess = this.api.callApiServiceMethod(url, parmasIssue);
    responseProcess.then((data) => {
      this.props.handleAPICall('averageissue');
      if (data.status === 'success' && data.statusCode === '200') {
        this.processChartData(data.data.closedissuechartData, data.data.openissuechartData, data.data.avgType);
      } else {
  ...
  render() {
    ...
    const tooltip = {
      contents(d) {
        ...
        let cont = '';
        if (typeName !== 'Multi') {
          cont = `<div class="toolTip"><span>Date: ${date} </span><span>Avg Issue ${lableText} Days : ${hrs} </span><span>Issue ${lableText} Count : ${count}</span></div>`;
        } else {
          cont = `<div class="toolTip"><span>Date: ${date} </span><span>Avg Issue Open Days : ${openhrs} </span><span>Issue Open Count : ${opencount}</span><span>Avg Issue Closed Days : ${closedhrs} </span><span>Issue Closed Count : ${closcount}</span></div>`;
        }
        return cont;
      }
    };
          ...
          <Row className="chartdiv">
            <Col className="chartdata">
              {!stateVal.showChart ? (
                ...
                <C3Chart data={data} legend={legend} tooltip={tooltip} axis={axis} padding={padding} />
              ) : (
...
export default AverageIssueAge;
